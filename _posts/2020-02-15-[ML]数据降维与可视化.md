...
layout: post
title: '[ML] 数据降维与可视化算法'
subtitle: 'PCA，SVD 与 t-SNE 算法'
date: 2020-02-15
categories: ML
cover: 
tags: math MachineLearning numpy
---
...
因为奇异值分解中的 $U$ 矩阵即为 $Y Y^T$ 的特征向量构成的矩阵，因此：
...
虽然在上面给出了公式推导，然而在 t-SNE 算法过程中，还是有许多细节。例如如何选择高斯分布的 $\sigma$ ，这里涉及到困惑度的概念，因为篇幅原因，就不展开介绍了。 sklearn 中已经内置了 t-SNE 的算法：[`sklearn.manifold.TSNE`](https://scikit-learn.org/stable/modules/generated/sklearn.manifold.TSNE.html) ，可以更加方便地使用 t-SNE 对数据进行降维。

> 如果想要更详细地了解 SNE ，对称 SNE 以及 t-SNE 的代码实现以及公式，可以参考 [t-SNE完整笔记](http://www.datakit.cn/blog/2017/02/05/t_sne_full.html)
...
例如，我们现在取出 man, woman, male 这三个词向量，将它绘制于平面上（为了直观起见）。现在我们建立了 man-woman ，以及 man-male 这两个关系，现在要推断 $?$ 位置的单词是什么，如下图所示：

![3](/assets/image/Dimensionality-Reduction-3.png)

对于人类来说，我们可能很容易就知道，这个单词是 female ，而对于计算机而言，如果我们对词向量进行适当的线性降维，那么计算机也就能很容易发现这样类似的关系。
...
在本文中，我主要介绍了 PCA 以及 t-SNE 两种常见的数据降维算法，并且也对其进行了详细地介绍。在实践中，PCA 算法由于计算速度快，因此广泛地应用于数据压缩、模型加速、特征分析等场景；而 t-SNE 算法则普遍用于数据可视化，一般很少做应用。除了 PCA 和 t-SNE 外，数据降维还有许许多多其他的算法，在这里就不展开介绍了。
...
在本文中，我主要介绍了 PCA 以及 t-SNE 两种常见的数据降维算法。在实践中，PCA 算法由于计算速度快，因此广泛地应用于数据压缩、模型加速、特征分析等场景；而 t-SNE 算法则普遍用于数据可视化，一般很少做其他应用。除了 PCA 和 t-SNE 外，数据降维还有许许多多其他的算法，在这里就不展开介绍了。